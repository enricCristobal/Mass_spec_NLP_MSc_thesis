...
def main(args):
    ...
    tokens_dir = '/home/projects/cpr_10006/projects/gala_ald/data/plasma_scans/BERT_tokens_' + str(args.peak_filter) + '/' + args.case + '_' + str(args.num_bins)  + '_tokens/'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--peak_filter', help='Top percentage of bins considered based on the highest intensity peaks', required=True, type=float)
    parser.add_argument('--case', help='Combination of parameters for tokens used', required=True)
    parser.add_argument('--input_size', help='Sentence length where natural BERT size is 512 (later size reductions will be done if needed)', required=True, type=int)
