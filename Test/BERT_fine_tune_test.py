...
finetune_samples = samples_list[:file_limit_finetune]
...
def fine_tune_ds(fine_tune_files, class_param = 'Group2', labels_path = '/home/projects/cpr_10006/projects/gala_ald/data/clinical_data/ALD_histological_scores.csv'):
    ...
    for i in range(len(labels_df)):
        labels_df['File name'].iloc[i] = labels_df['File name'].iloc[i][beginning_file_name.iloc[i]:-18]  ## WORKS FINE?
...
def train(model: nn.Module) -> None:
    ...
    for batch in range(num_batches):
        ...
        loss = criterion(output[0], class_labels[batch].to(device)) # can be done for all the samples of the batch at a time, but shape of input (output of the model in our case)
...
def evaluate(model: nn.Module) -> float:
    ...
    with torch.no_grad():
        for batch in range(num_batches):
            output = model(inputs[batch].to(device))
            loss = criterion(output[0], class_labels[batch].to(device))
