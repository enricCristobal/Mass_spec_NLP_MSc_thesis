...
activation = "gelu" # activation function on the intermediate layer (For now with gelu is not working)
...
def train(model: nn.Module) -> None:
    ...
    for batch in range(num_batches):
        ...
        output = model(inputs[batch], src_mask[batch]) #.to(device), src_mask[batch].to(device))
