...
# Define voabulary
num_bins = 50000 #!!!CHANGE TO 10000
...
model = TransformerModel(ntoken=len(vocab),  # size of vocabulary 
                        ...
                        nhead= 2, #3, #6, # number of nn.TransformerEncoderLayer in nn.TransformerEncoder
                        nlayers= 2,#3, #6, # number of heads in nn.MultiheadAttention
...
for epoch in range(1, n_epochs + 1):
    ...
    train_loss = BERT_train(model, optimizer, criterion, scheduler, dataset=train_ds, results_file=evolution_file, \
        ...
        shorter_len_perc=perc_epochs_shorter, log_interval=1, device=device, scaler=scaler)
