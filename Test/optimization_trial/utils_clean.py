...
def BERT_train(model: nn.Module, optimizer, criterion, scheduler, dataset: list, vocab, results_file, batchsize: int, current_epoch: int, total_epochs: int, \
    ...
    inputs, targets, src_mask, labels, num_batches = get_training_batch(dataset, vocab, batchsize, epoch_status, limited_seq_len, shorter_len_perc)
...
def BERT_evaluate(model: nn.Module, criterion, dataset: list, vocab, results_file, batchsize: int, current_epoch: int, total_epochs: float, \
...
def get_training_batch(data_ds, vocab, batchsize, epoch_status, limited_seq_len, shorter_len_perc):
    ...
    # undo the incorporation of 0.5 for the labels to optimize .to(device) calls
    for spectra in range(len(labels)): 
        labels[spectra] = torch.masked_select(labels[spectra], labels[spectra] != (len(vocab) + 1)) 
