    ...
    for filename in os.listdir(input_dir): 
        if filename != '20190525_QE10_Evosep1_P0000005_LiNi_SA_Plate3_A7.raw.pkl': # Giving issues after manual inspection
            ...
            for i in range(len(x)):
                ...
                if CLS_token:
                    sentence.extend(cls_token)
                
                if add_ret_time:
                    sentence.extend(ret_time)
                ...
                # Collect
                sentence.extend(tokens_string)
                
                # Maximum input size for BERT is 512 
                if len(sentence) > (input_size - 1): # -1 as '[EOS]' token will be added later 
                    sentence = sentence[:(input_size -1)] 
...
def create_training_dataset(sample, vocab, samples_dir,
            ...
            if random.random() < 0.15: # 15% tokens will be masked
                if input[i+j] != '[PAD]' and input[i+j] != '[EOS]':
                    ...
                    if second_random < 0.8: # of the 15%, 80% will be changed to [MASK] token
                        ...
                        input[i+j] =  str(random.randint(0,(len(vocab))))  
    ...
    for input_line in input:
        for input, masked_indices in mask_input(input_line, CLS_token, add_ret_time):
            ...
            #if len(masked_indices) > max_len_mask:
            #    max_len_mask = len(masked_indices)
    ''' 
    for spectra in range(len(mask_indices)):
        mask_indices[spectra] = torch.tensor(mask_indices[spectra])
        if len(mask_indices[spectra]) < max_len_mask:
            mask_indices[spectra] = torch.cat([mask_indices[spectra], torch.tensor([0.5]*(max_len_mask - len(mask_indices[spectra])))])
    '''
    # Process of making labels the same size as the other tensors to allow optimization of training by reducing .to(device) calls and
    # passing all data at once
    for spectra in range(len(mask_indices)):
        mask_indices[spectra].extend([len(vocab) + 1]*(input_size - len(mask_indices[spectra]))) 
        # first idea was to extend with 0.5 since a mask index would never be float, but then it gave problems when training the model with tnesor type float
        # extend with value outside our vocab to keep tensor.type Long
        mask_indices[spectra] = torch.tensor(mask_indices[spectra])
    ...
    return torch.stack(input_tensor_list), torch.stack(target_tensor_list), torch.stack(attention_mask_tensor), torch.stack(mask_indices)


def TrainingBERTDataLoader(files_dir, vocab, training_percentage: float, validation_percentage: float, CLS_token: bool,\
     add_ret_time: bool, data_repetition: int = 1, device: str = 'cpu'):
    ...
    for sample in range(len(train_ds)):
        train_ds[sample] = torch.stack(train_ds[sample])
    ...
    for sample in range(len(val_ds)):
        val_ds[sample] = torch.stack(val_ds[sample])

    return torch.stack(train_ds).to(device), torch.stack(val_ds).to(device)
...
def FineTuneBERTDataLoader(files_dir: str, vocab, training_percentage: float, validation_percentage: float, class_param: str = 'Group2', \
    kleiner_type = None, labels_path: str = '/home/projects/cpr_10006/projects/gala_ald/data/clinical_data/ALD_histological_scores.csv'):
