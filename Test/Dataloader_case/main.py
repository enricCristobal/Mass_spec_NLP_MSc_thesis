...
from Dataloader import data_load, create_dataset
...
'''
num_bins = 50000
input_dir_pickle = os.getcwd() + '\\pickle_data\\'
output_dir_pickle = os.getcwd() + '\\testing_data\\'

data = data_load(output_dir_pickle, num_bins)

data.pickle_to_df(input_dir_pickle)
...
output_dir = os.getcwd() + '\\test_data\\'
input_dir = os.getcwd() + '\\testing\\'
...
#data.pickle_to_df(input_dir)
fn_train = ['20190514_QE10_Evosep1_P0000005_LiNi_SA_Plate1_A1.raw', '20190517_QE10_Evosep1_P0000005_LiNi_SA_Plate2_A1.raw'] 
train_ds = [create_dataset(output_dir, f, vocab, num_bins) for f in fn_train]
dataset_train = ConcatDataset(train_ds)
train_loader = DataLoader(dataset=dataset_train, batch_size=batchsize, drop_last=True, shuffle=True, num_workers=n_workers)

'''
# If pickle files need to be converted into raw data text files
pickle_files = True # we can add some sys.arg when calling this code to specify if we want this step to 
...
def evaluate(model: nn.Module) -> float:
    ...
    inputs, targets, src_mask, labels, num_batches = get_batch(input_val_ds, target_val_ds, att_mask_val_ds, labels_val_ds, batchsize)
    with torch.no_grad():
        for batch in range(num_batches):
            output = model(inputs[batch].to(device), src_mask[batch].to(device))
            loss = 0
...
plt.savefig('/home/projects/cpr_10006/people/enrcop/BERT_error.png')
