...
class data_load:
    ...
    def unpickle_to_raw(self, input_dir, add_ret_time = True, descending_order = False):
        ...
        for filename in os.listdir(input_dir):
            ...
            for i in range(len(x)):
                #ret_time = x.iloc[i,0] # but adding retention time makes vocab definition as complex as if we added intensities
...
class create_dataset(Dataset):
        ...
        def __init__(self, file_dir, sample_file, vocab, num_bins, input_size = 512):
            ...
            def yield_tokens(sample):
                ...
                fh = open(self.file_dir + sample, 'r')
                for line in fh:
            ...
            for input_yield, target_yield, att_mask_yield in yield_tokens(sample_file):
            ...
            self.target = target_tensor_list
            self.att_mask = attention_mask_tensor

            # helper function to generate masked input
            def mask_input(input):
                masked_indices = []
                for i in range(len(input)-3): # To not mask the ret_time nor the [SEP], [EOS] tokens
                    if input[i+2] != '[PAD]': # +2 to avoid ret_time and [Sep] tokens
                        if random.random() < 0.15:
                            ...
                            if second_random < 0.8:
                                ...
                                input[i+2] =  str(random.randint(0,self.num_bins - 1))  
                                masked_indices.append(i+2) 
                            else:
                                masked_indices.append(i+2)             
            ...
            self.input = input_tensor_list
            self.mask_indices = mask_indices
            #print(len(self.input), len(self.target), len(self.att_mask), len(mask_indices))
        ...
        def __getitem__(self):
            for i in range(len(self.input)):
                return(self.input[i], self.target[i], self.att_mask[i], self.mask_indices[i])
