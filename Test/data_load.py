...
as well as the final DataLoaders with all the required tensors (inputs, targets, padding masks, labels, etc.) ready to be passed
to our models will be done."""
...
def TrainingBERTDataLoader(files_dir, vocab, training_percentage: float, validation_percentage: float, CLS_token: bool,\
     add_ret_time: bool, input_size: int = 512, data_repetition: int = 1):
...
def compensate_imbalance(labels_list, samples_list):
    ...
    #selection = random.choices(list(enumerate(potential_repeat_samples)), k=(n_bigger_class - n_smaller_class))
